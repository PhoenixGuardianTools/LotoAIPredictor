...
def generate_model_e_grid(game_config, history_df, draw_date=None):
    ...
    def smart_draw(pool, freq_map, pattern_bonus, bias_map, fractal_map, theory_map, period_map, bayes_map, markov_map, nn_map, anomaly_map, adjust_map, count):
        weights = []
        for n in pool:
            ...
            anomaly = anomaly_map.get(n, 1)
            adjust = adjust_map.get(n, 1)
            bonus = 2 if n in pattern_bonus else 1
            
            # Pondération dynamique optimisée
            weight = freq * bias * bonus * fractal * theory * period * bayesian * markov * neural * anomaly * adjust
    ...
    nums = smart_draw(game_config['pool'], adjusted_freq, pattern_nums, positive_bias, fractal_bonus, game_theory_weights, period_trends, bayesian_weights, markov_predictions, neural_weights, anomaly_correction, loto_adjustments, game_config['numbers'])
